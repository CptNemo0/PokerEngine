...
CombinationType HandEvaluator::Evaluate(std::span<const model::Card> hand) {
...
HandEvaluator::CheckRoyalFlush(hand_type hand) const {
  ...
  auto contains = [&](model::Card::Rank rank) {
    return std::find(hand.begin(), hand.end(),
                     model::Card{potential_flush_suit.value(), rank}) !=
           hand.end();
  };

  return contains(model::Card::Rank::kAce) &&
                 contains(model::Card::Rank::kKing) &&
                 contains(model::Card::Rank::kQueen) &&
                 contains(model::Card::Rank::kJack) &&
                 contains(model::Card::Rank::kTen)
